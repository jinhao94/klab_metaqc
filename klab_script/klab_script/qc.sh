...
function display_help() {
    ...
    echo "Note1: There are following types host: | human | c57mouse | c57mouse_metatran | None (means no host) | pig | sheep"
    ...
    echo "   -t, --host_type            host type | human | c57mouse | c57mouse_metatran | None (means no host) | pig | sheep"
...
snakemake -s /ddnstor/imau_sunzhihong/mnt1/script/script/qc.snakemake.py --config workdir=${outfolder} db_path=${host_path} host=${type} file_names_txt=$PWD/${new_sample_list} -r -p --cores $threads -j $threadss
